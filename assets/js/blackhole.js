/* -------------------------------------------------------------------------- *
 *  Black hole hero background — vanilla WebGL.
 *
 *  Ported from a React/Tailwind component. The shaders and the physics are
 *  unchanged; only the React wrapper is replaced, because this site is static
 *  HTML with no build step.
 *
 *  Every pixel fires one ray out of the camera and walks it backwards through
 *  curved space until it falls through the horizon, leaves for the stars, or
 *  cuts the gas disc — often several times, because a ray can loop the hole.
 *  The disc is drawn once; the halo above it and the thin ring hugging the
 *  shadow are that same disc seen again through bent light.
 *
 *  r = 1 is the event horizon, so the photon sphere lands at 1.5 and the disc
 *  starts at 3. Those numbers fall out of the metric, they are not tuned.
 * -------------------------------------------------------------------------- */
...
export function initBlackHole(canvas, options) {
  ...
  /* A phone has no room to stand the art beside the copy, so the arrangement
     turns through 90°: hole low and whole, veil from the top, and a wider
     field to win back the room the narrow frame lost. It also pays for every
     ray, hence the lower step and resolution counts. */
  const C = Object.assign({}, base);
  function applyBreakpoint() {
    Object.assign(C, base);
    if (narrowQuery.matches) Object.assign(C, base.narrow);
  }
  applyBreakpoint();
  ...
  /* Take the canvas out of the picture rather than leave a dead one behind: a
     canvas whose context has died paints white over the background, which is
  ...
  /* One still, properly: nothing is animating, so the frames the running
     average would collect over time are taken all at once instead. */
  ...
  /* Off screen it stops drawing entirely — the hero is one screen tall and the
     page is seven, so this shader is idle for most of a visit. */
...
/* -------------------------------------------------------------------------- *
 *  Wire it to the hero, in the site's champagne-gold palette.
 * -------------------------------------------------------------------------- */
